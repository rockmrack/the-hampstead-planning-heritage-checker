/**
 * Acoustic Design Assessment API
 * 
 * Analyzes noise and acoustic considerations for proposed developments
 */

import { NextRequest, NextResponse } from 'next/server';
import acousticDesign from '@/lib/services/acoustic-design';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const assessment = await acousticDesign.assessAcoustics(
      address,
      postcode,
      projectType,
      projectDetails || {}
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        service: 'acoustic-design-assessment'
      }
    });
  } catch (error) {
    console.error('Acoustic Design API error:', error);
    return NextResponse.json(
      { error: 'Failed to generate acoustic design assessment' },
      { status: 500 }
    );
  }
}
    ...
    return NextResponse.json(
      { 
        ...
        usage: {
          ...
          example: '/api/acoustic-design?address=10%20Finchley%20Road&postcode=NW3%205HT&projectType=extension',
          projectDetails: {
            nearRoad: 'boolean - is property near a busy road',
            nearRailway: 'boolean - is property near railway',
            plantEquipment: 'boolean - will development include mechanical plant',
            newDwellings: 'boolean - are new separate dwellings being created'
          }
        }
      },
      { status: 400 }
