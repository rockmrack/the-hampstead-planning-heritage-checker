...
export async function POST(request: NextRequest) {
  try {
    ...
    const assessment = await fireSafety.assessFireSafety(
      address,
      postcode,
      projectType,
      projectDetails || {}
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        service: 'fire-safety-strategy'
      }
    });
  } catch (error) {
    console.error('Fire Safety API error:', error);
    return NextResponse.json(
      { error: 'Failed to generate fire safety assessment' },
      { status: 500 }
    );
  }
}
    ...
    return NextResponse.json(
      { 
        ...
        usage: {
          ...
          example: '/api/fire-safety?address=10%20Church%20Row&postcode=NW3%206UP&projectType=loft',
          projectDetails: {
            storeys: 'number - total storeys including new',
            openPlanDesign: 'boolean - open plan kitchen/living',
            existingDetection: 'boolean - has smoke detectors'
          }
        }
      },
      { status: 400 }
