...
export async function POST(request: NextRequest) {
  try {
    ...
    const analysis = await townscape.analyzeTownscape(
      address,
      postcode,
      projectType,
      projectDetails || {}
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        service: 'townscape-analysis'
      }
    });
  } catch (error) {
    console.error('Townscape Analysis API error:', error);
    return NextResponse.json(
      { error: 'Failed to generate townscape analysis' },
      { status: 500 }
    );
  }
}
    ...
    return NextResponse.json(
      { 
        ...
        usage: {
          ...
          example: '/api/townscape-analysis?address=10%20Church%20Row&postcode=NW3%206UP&projectType=extension',
          characterAreas: ['Hampstead Village', 'Heath Fringe', 'Hampstead Garden Suburb', 'Belsize Park']
        }
      },
      { status: 400 }
    );
